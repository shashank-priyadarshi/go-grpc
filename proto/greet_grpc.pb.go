...
type GreetServiceClient interface {
	...
	SayHelloBidirecStreaming(ctx context.Context, opts ...grpc.CallOption) (GreetService_SayHelloBidirecStreamingClient, error)
}
...
func (c *greetServiceClient) SayHelloBidirecStreaming(ctx context.Context, opts ...grpc.CallOption) (GreetService_SayHelloBidirecStreamingClient, error) {
	stream, err := c.cc.NewStream(ctx, &GreetService_ServiceDesc.Streams[2], "/greet_service.GreetService/SayHelloBidirecStreaming", opts...)
	if err != nil {
		return nil, err
	}
	x := &greetServiceSayHelloBidirecStreamingClient{stream}
	return x, nil
}

type GreetService_SayHelloBidirecStreamingClient interface {
	Send(*HelloRequest) error
	Recv() (*HelloResponse, error)
	grpc.ClientStream
}

type greetServiceSayHelloBidirecStreamingClient struct {
	grpc.ClientStream
}

func (x *greetServiceSayHelloBidirecStreamingClient) Send(m *HelloRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *greetServiceSayHelloBidirecStreamingClient) Recv() (*HelloResponse, error) {
	m := new(HelloResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type GreetServiceServer interface {
	...
	SayHelloBidirecStreaming(GreetService_SayHelloBidirecStreamingServer) error
...
func (UnimplementedGreetServiceServer) SayHelloBidirecStreaming(GreetService_SayHelloBidirecStreamingServer) error {
	return status.Errorf(codes.Unimplemented, "method SayHelloBidirecStreaming not implemented")
}
...
func _GreetService_SayHelloBidirecStreaming_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(GreetServiceServer).SayHelloBidirecStreaming(&greetServiceSayHelloBidirecStreamingServer{stream})
}

type GreetService_SayHelloBidirecStreamingServer interface {
	Send(*HelloResponse) error
	Recv() (*HelloRequest, error)
	grpc.ServerStream
}

type greetServiceSayHelloBidirecStreamingServer struct {
	grpc.ServerStream
}

func (x *greetServiceSayHelloBidirecStreamingServer) Send(m *HelloResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *greetServiceSayHelloBidirecStreamingServer) Recv() (*HelloRequest, error) {
	m := new(HelloRequest)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var GreetService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "SayHelloBidirecStreaming",
			Handler:       _GreetService_SayHelloBidirecStreaming_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
